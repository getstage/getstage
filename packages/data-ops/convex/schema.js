...
const userRole = v.union(v.literal("freelancer"), v.literal("studio"), v.literal("in-house"), v.literal("agency"));
const plan = v.union(v.literal("free"), v.literal("pro"));
const billingProvider = v.union(v.literal("stripe"), v.literal("polar"), v.literal("creem"), v.literal("unknown"));
const paymentProvider = v.union(v.literal("stripe"), v.literal("unknown"));
const paymentAccessMode = v.union(v.literal("restricted_key"), v.literal("connect"));
const paymentConnectionStatus = v.union(v.literal("pending"), v.literal("active"), v.literal("error"), v.literal("disconnected"));
const sheetSourceType = v.union(v.literal("google_sheet"), v.literal("csv_upload"));
const projectType = v.union(v.literal("branding"), v.literal("web-design"), v.literal("product-design"), v.literal("app-design"), v.literal("web-app"), v.literal("packaging"), v.literal("motion-design"), v.literal("illustration"), v.literal("other"));
const projectStatus = v.union(v.literal("active"), v.literal("paused"), v.literal("completed"));
const phaseStatus = v.union(v.literal("completed"), v.literal("active"), v.literal("upcoming"));
const taskPriority = v.union(v.literal("low"), v.literal("medium"), v.literal("high"));
const attachmentType = v.union(v.literal("image"), v.literal("pdf"), v.literal("document"), v.literal("other"));
const generatedDesignProvider = v.literal("stitch");
const designConnectionStatus = v.union(v.literal("active"), v.literal("error"), v.literal("archived"));
const generatedDesignSource = v.union(v.literal("stage_proxy"), v.literal("user_sync"));
const generatedDesignStatus = v.union(v.literal("ready"), v.literal("error"));
const stitchDeviceType = v.union(v.literal("DEVICE_TYPE_UNSPECIFIED"), v.literal("MOBILE"), v.literal("DESKTOP"), v.literal("TABLET"), v.literal("AGNOSTIC"));
const stitchModelId = v.union(v.literal("MODEL_ID_UNSPECIFIED"), v.literal("GEMINI_3_PRO"), v.literal("GEMINI_3_FLASH"));
const uploadPurpose = v.union(v.literal("task-attachment"), v.literal("csv-upload"), v.literal("profile-avatar"), v.literal("client-avatar"), v.literal("project-marker"), v.literal("portal-logo"), v.literal("generated-design"), v.literal("project-asset"), v.literal("research-refero"), v.literal("research-brief"), v.literal("moodboard-upload"), v.literal("moodboard-refero"), v.literal("moodboard-figma"), v.literal("moodboard-url"));
const subscriptionStatus = v.union(v.literal("active"), v.literal("trialing"), v.literal("cancelling"), v.literal("past_due"), v.literal("unpaid"), v.literal("incomplete"), v.literal("canceled"), v.literal("expired"));
const invoiceStatus = v.union(v.literal("draft"), v.literal("open"), v.literal("paid"), v.literal("overdue"), v.literal("void"));
const paymentStatus = v.union(v.literal("pending"), v.literal("succeeded"), v.literal("failed"));
const importRunStatus = v.union(v.literal("running"), v.literal("success"), v.literal("error"));
const financeSource = v.union(v.literal("stripe_connect"), v.literal("google_sheet"), v.literal("csv_upload"));
const financeEntryType = v.union(v.literal("invoice"), v.literal("payment"), v.literal("expense"), v.literal("refund"), v.literal("adjustment"));
const financeDirection = v.union(v.literal("incoming"), v.literal("outgoing"));
const financeStatus = v.union(v.literal("draft"), v.literal("pending"), v.literal("paid"), v.literal("overdue"), v.literal("failed"));
const agentProvider = v.literal("claude");
const agentClient = v.literal("claude_code");
const agentConnectionMode = v.literal("skill");
const agentConnectionStatus = v.union(v.literal("pending"), v.literal("connected"), v.literal("error"), v.literal("disconnected"));
const agentConnectionSource = v.union(v.literal("onboarding"), v.literal("settings"));
const claudeToolAvailability = v.union(v.literal("unknown"), v.literal("claimed"));
const aiProvider = v.literal("anthropic");
const aiProviderCredentialLabel = v.literal("claude");
const aiProviderCredentialStatus = v.union(v.literal("untested"), v.literal("valid"), v.literal("invalid"));
const aiRunModule = v.union(v.literal("research"), v.literal("strategy"), v.literal("flows"), v.literal("moodboard"), v.literal("generate"), v.literal("delivery"));
const aiRunStatus = v.union(v.literal("draft"), v.literal("running"), v.literal("completed"), v.literal("failed"), v.literal("needs_input"));
const aiRunTrigger = v.union(v.literal("user"), v.literal("agent"));
const aiArtifactStatus = v.union(v.literal("draft"), v.literal("ready"), v.literal("approved"), v.literal("superseded"), v.literal("failed"));
const aiArtifactContentFormat = v.union(v.literal("markdown"), v.literal("json"), v.literal("link_set"));
const artifactDestinationProvider = v.union(v.literal("notion"), v.literal("figma"));
const artifactDestinationStatus = v.union(v.literal("requested"), v.literal("in_progress"), v.literal("completed"), v.literal("failed"));
const artifactDestinationRequestedVia = v.union(v.literal("claude"), v.literal("native"));
const nativeIntegrationProvider = v.union(v.literal("notion"), v.literal("figma"));
const nativeIntegrationStatus = v.union(v.literal("pending"), v.literal("active"), v.literal("error"), v.literal("disconnected"));
const figmaExportStatus = v.union(v.literal("requested"), v.literal("claimed"), v.literal("completed"), v.literal("failed"));
export default defineSchema({
    ...
    tasks: defineTable({
        ...
        boardStatus: v.optional(v.union(v.literal("backlog"), v.literal("todo"), v.literal("in-progress"), v.literal("done"))),
        order: v.number(),
        createdAt: v.number(),
        updatedAt: v.number(),
    })
