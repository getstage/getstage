...
export const exportFlowsToFigJamArgs = {
  projectId: v.id("projects"),
  artifactId: v.id("projectAiArtifacts"),
};

export async function exportFlowsToFigJamHandler(
  ctx: MutationCtx,
  args: { projectId: Id<"projects">; artifactId: Id<"projectAiArtifacts"> },
) {
  const { user } = await requireProjectAccess(ctx, args.projectId);
  const artifact = await getArtifactRecord(ctx, args.artifactId);
  ...
  if (!connection || connection.status !== "active") {
    throw new Error("Connect Figma in Settings before sending flows to FigJam.");
  }

  const timestamp = now();
  const message =
    "Figma is connected, but creating FigJam canvas content requires a Figma Plugin/Widget export path. Stage saved the export request and will support canvas push when that integration is available.";
  const existingDestination = (
    ...
    .filter(
      (destination) =>
        destination.provider === "figma" && destination.action === "flows_figjam_export",
    )
    .sort((a, b) => b.updatedAt - a.updatedAt)[0];

  const patch = {
    status: "failed" as const,
    destinationLabel: "FigJam flow map",
  ...
  const destinationId = existingDestination
    ...
    : await ctx.db.insert("artifactDestinations", {
        ...
        projectId: args.projectId,
        provider: "figma",
        action: "flows_figjam_export",
        createdAt: timestamp,
        ...patch,
      });

  if (existingDestination) {
    await ctx.db.patch(existingDestination._id, patch);
  }
