...
import {
  cancelReasonFromStripeSubscription,
  trialStartedAtMsFromStripe,
...
export async function createCheckoutSessionHandler(
  ...
  if (requestedSeats !== includedSeats) {
    throw new Error(`This plan includes exactly ${includedSeats} seats.`);
  }
...
export async function backfillSubscriptionMirrorsHandler(ctx: ActionCtx) {
  ...
  while (true) {
    ...
    for (const subscription of page.data) {
      ...
      const trialStartedAtMs = trialStartedAtMsFromStripe(subscription);
