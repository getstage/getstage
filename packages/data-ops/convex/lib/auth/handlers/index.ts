...
import { getEnv, getSiteUrl } from "../../../helpers/env";
...
export async function redirectAfterSignIn({ redirectTo }: { redirectTo: string }) {
...
function isDemoAuthEnabled() {
  ...
  const siteHostname = getHostnameFromUrl(getEnv("SITE_URL")) ?? getHostnameFromUrl(getEnv("CONVEX_SITE_URL"));
...
function applyUserDefaults(args: {
  ...
  const fallbackName = args.normalizedEmail ? buildNameFromEmail(args.normalizedEmail) : "Stage User";
...
export const authProviders = DEMO_AUTH_ENABLED ? [Google, LoopsOTP, Demo] : [Google, LoopsOTP];
