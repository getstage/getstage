...
export function DashboardPage() {
  ...
  const isLoading = Boolean(user?.email) && dashboardData === undefined;
  const projects = dashboardData?.projects ?? [];
  const paymentSummary = dashboardData?.paymentSummary ?? null;
  ...
  const receivedTotal = paymentSummary?.receivedTotal ?? 0;
  const outstandingTotal = paymentSummary?.outstandingTotal ?? 0;
  const outstandingDisplay = formatCurrencyDisplay(outstandingTotal);
  const receivedDisplay = formatCurrencyDisplay(receivedTotal);
  const pendingDisplay = formatCurrencyDisplay(paymentSummary?.pendingTotal ?? 0);
