...
function getGroupingThreshold(horizon: TimelineHorizon) {
...
export function Timeline({ projects, horizon = "this-month" }: TimelineProps) {
  ...
  const layout = useMemo(() => {
    ...
    const threshold = getGroupingThreshold(horizon);
    ...
    return {
      ...
      visibleProjects,
      groups,
      todayX: showToday ? (todayPct / 100) * width : null,
    };
  ...
  const handleMouseMove = (clientX: number) => {
    ...
    const tipInset = Math.min(100, layout.width / 2);

    setTracking({
      x,
      curveTop,
      dateLabel: SHORT_DATE_FORMATTER.format(new Date(dateTimestamp)),
      dateFull: FULL_DATE_FORMATTER.format(new Date(dateTimestamp)),
      activeProjects,
      tipLeft: clamp(x, tipInset, layout.width - tipInset),
  ...
  const profileHoverDetails = useMemo(() => {
    ...
    const left = clamp(profileHover.x + 18, 8, layout.width - TOOLTIP_WIDTH - 8);
  ...
  return (
    <section className="relative flex min-h-[35vh] items-center justify-center px-4 py-[50px]">
      <div
        ref={regionRef}
      ...
      >
        ...
        {layout.todayX !== null ? (
          <div
            className="pointer-events-none absolute top-0 z-[2] w-px bg-[rgba(135,130,245,0.3)]"
            style={{ left: `${layout.todayX}px`, height: `${CURVE_HEIGHT}px` }}
          />
        ) : null}

        {layout.groups.map((group) => {
          const groupLeft = (group.pct / 100) * layout.width;
        ...
        {profileHover ? (
          <>
            <div
              className="pointer-events-none absolute top-0 z-[3] w-px bg-accent/22 transition-opacity duration-200"
        ...
        {tracking && !hoveredProjectId ? (
          <>
            <div
              className="pointer-events-none absolute top-0 z-[3] bg-text-primary"
              style={{ left: `${tracking.x}px`, width: "1.5px", height: `${CURVE_HEIGHT}px` }}
            />
            <div
              className="pointer-events-none absolute z-[3] h-2 w-2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-accent"
              style={{ left: `${tracking.x}px`, top: `${tracking.curveTop}px` }}
            />
            <div
              className="pointer-events-none absolute z-20 -translate-x-1/2 whitespace-nowrap rounded-[6px] bg-text-primary px-[14px] py-2 text-[13px] font-medium text-white"
              style={{ left: `${tracking.x}px`, top: `${CURVE_HEIGHT + 8}px` }}
            >
              {tracking.dateLabel}
            </div>
            ...
            {tracking.activeProjects.length > 0 ? (
              <div
                className="pointer-events-none absolute z-[15] min-w-[180px] -translate-x-1/2 -translate-y-full rounded-[12px] border border-border bg-white px-4 py-3 shadow-[0_4px_20px_rgba(0,0,0,0.08)]"
                style={{ left: `${tracking.tipLeft}px`, top: `${tracking.tipTop}px` }}
              >
