...
| **P0 wireframes: provider reliability reset** | **RAG and rendering work, but generation is not accepted as reliable.** A live GLM run returned an upstream `503`; the current atomic multi-screen response converted that into a failed chunk. The target is one Stage batch with isolated per-screen calls/results, shared contracts for Claude/Codex/Nebius, exact error classes, bounded fresh retries, no model-repair loop, smaller model context, full renderer dependency closure, and `partial_success` persistence. No provider becomes the default before the fixed parity fixtures pass. **Plan:** `docs/RAG/WIREFRAMES_RIG_NEBIUS_GATEWAY_PLAN_28-AUG.md`; **code map:** `docs/RAG/WIREFRAMES_GENERATION_CODEMAP.md`. | `apps/wireframe-ai-gateway/`, `apps/stage-engine/src/wireframes/`, `packages/wireframe-renderer/` |
