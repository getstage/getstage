...
export function createDesktopAuthCallbackServer({
  ...
  return {
    ...
    async start() {
      ...
      await new Promise<void>((resolve, reject) => {
        const nextServer = createServer(createRequestHandler());
    ...
    stop() {
      void closeServer().catch((error: unknown) => {
        const message = error instanceof Error ? error.message : "Unknown callback server stop error.";
        console.warn(`[stage-auth] local desktop auth callback server stop failed: ${message}`);
      });
    },
  };
}
