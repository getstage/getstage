...
const CLAUDE_INSTALL_COMMAND = "npx skills add getstage/agent-mode";
...
export function useOnboardingController({
  ...
  const continueEnabled = canContinue({
    ...
    hasClientAvatar: Boolean(draft.pendingAvatarFile || draft.clientAvatar),
  ...
  useEffect(() => {
    if (step !== "claude" || claudeState?.connection || !isConvexAuthenticated) {
      return;
    }

    void createPendingConnection({ source: "onboarding" }).catch(() => undefined);
  }, [step, claudeState?.connection, createPendingConnection, isConvexAuthenticated]);
  ...
  useEffect(() => {
    ...
    async function runCreation() {
      try {
        if (submission.createProject && isConvexAuthenticated) {
  ...
  async function handlePaywallUpgrade(billingCycle: "monthly" | "yearly") {
    ...
    if (!isConvexAuthenticated) {
      setCheckoutError("Checkout is not available until the desktop session is fully authenticated.");
      setIsCheckoutLoading(false);
      return;
    }
