import type { DashboardProject, DashboardTask } from "@/models/dashboard/dashboard";
...
export function getProjectTasks(project: DashboardProject, tasks: DashboardTask[]) {
  return tasks
    .filter((task) => (task.projectId === project.id || task.projectName === project.name) && !task.isCompleted)
    .sort((a, b) => {
