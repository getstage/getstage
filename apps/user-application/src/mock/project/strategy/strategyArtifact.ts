...
function buildStrategyArtifact(projectId: string, generatedAt: number): StrategyArtifact {
  return {
    apiVersion: "v1",
        ...
        boxes: [
          ...
          },
          {
            ...
            bullets: [
              ...
              "Hide empty states that educate and activate",
              "Dark mode as a cosmetic toggle",
            ],
          },
          {
            title: "AVOID",
            bullets: ["Sidebar navigation on mobile", "Feature-gated dashboards", "Generic wellness look"],
          },
        ],
      },
      {
        ...
        cards: [
          ...
          {
            title: "Onboarding",
            objective: "Activate new teams without training overhead",
            kpi: "Setup completion rate",
            keyElement: "Guided checklist with one primary action per step.",
          },
          {
            title: "Reports",
            objective: "Help finance teams export insights quickly",
            kpi: "Export rate, time-to-report",
            keyElement: "Prebuilt views with editable filters and clear empty states.",
          },
        ],
      },
      {
...
export const MOCK_STRATEGY_PROJECT_ID = "mock-stellar-site";

const generatedAt = Date.parse("2025-04-06T14:00:00.000Z");

export const mockStrategyArtifact = strategyArtifactSchema.parse(
  buildStrategyArtifact(MOCK_STRATEGY_PROJECT_ID, generatedAt),
);

export function createMockStrategyArtifact(projectId: string, researchArtifactId?: string): StrategyArtifact {
  return strategyArtifactSchema.parse({
    ...buildStrategyArtifact(projectId, generatedAt),
    researchArtifactId,
  });
}
