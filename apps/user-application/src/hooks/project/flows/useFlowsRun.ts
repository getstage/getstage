...
const DEFAULT_FLOWS_MODELS: Record<ProviderId, string> = {
    ...
    async (providerId: ProviderId, source?: string) => {
      ...
      const runPromise = (async () => {
        ...
        if (!providerPreferences.isProviderEnabled(providerId)) {
          throw new Error(
            `Connect ${providerId === "claude" ? "Claude" : "Codex"} in Settings -> Integrations before running Flows.`,
          );
        }
        ...
        await providerRun.startRun.mutateAsync({
          providerId,
          modelId: DEFAULT_FLOWS_MODELS[providerId],
          prompt: FLOWS_PROMPT,
          mode: "flows",
