...
- [x] `prod-v0.2.35` completed successfully in GitHub Actions (run `34943255424`) with arm64/x64 DMGs and a published GitHub Release. Wireframes use the project's Flows screens with no generic fallback; packaged YouTube requests identify Stage and the modal includes an external fallback.
- [x] Stage Engine strict Clippy baseline restored: zero warnings, 169 Rust tests pass, and future desktop releases block on `cargo clippy --all-targets --all-features --locked -- -D warnings`.
