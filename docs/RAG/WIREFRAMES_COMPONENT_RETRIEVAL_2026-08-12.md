...
## What Stage did at the time of this decision

At the time of this decision, Stage could use specific components only when they had already been copied into and registered in `packages/wireframe-renderer`. This legacy runtime was removed during the AUG26 RAG cutover; the examples below are historical evidence, not current implementation guidance.
...
Stage already uses Convex, and Convex already provides native vector indexes. For hundreds or thousands of verified components and templates, one application-owned vector table plus R2 is the simpler first architecture; the document-oriented RAG component is unnecessary for one bounded vector per component.
...
Used for sanitized embedding latency, usage, and failure observability in the Convex cloud boundary. It helps diagnose retrieval but does not improve designs by itself. The local Claude/Codex CLI subprocesses remain visible through device-local Rust tracing because a shared LangSmith secret must not be shipped in the desktop application.
